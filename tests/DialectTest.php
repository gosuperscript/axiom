...
final class DialectTest extends TestCase
{
    ...
    public function composition_preserves_the_whole_core_rule_list(): void
    {
        ...
        // last — must survive composition intact.
    ...
    public function row_ambiguity_is_detected_behind_non_row_rules(): void
    {
        ...
        Dialect::core()->with(new class extends Extension {
            public function operators(): array
            {
                // A hand-written (non-row) rule standing ahead of the rows,
                // like the equality rule does in the full core dialect.
                $nonRow = new class implements \Superscript\Axiom\Operators\OperatorOverloader {
                    public function resolve(string $operator, \Superscript\Axiom\Types\Type $left, \Superscript\Axiom\Types\Type $right): \Superscript\Monads\Result\Result
                    {
                        return \Superscript\Monads\Result\Err(new \Superscript\Axiom\Types\TypeMismatch('Not mine.', unhandled: true));
                    }
                };
