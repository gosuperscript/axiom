...
It is not itself a `Source`: without parameter types it has no type or evaluation. The owning source compiler supplies those types to `SourceCompilation::scope()`. Keeping the names structurally beside the body lets `Expression::parameters()` and definition-cycle analysis exclude bound symbols while retaining every genuinely free symbol. In the example, `candidate` is local and `threshold` is captured from the enclosing expression.
