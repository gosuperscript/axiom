...
- compile every semantic child through `child()`, `children()`, `scope()`, `reference()`, or `symbol()`; this compiler descent supplies parameter and definition-cycle analysis, so property visibility has no structural meaning;
- use `reference()` for a rooted reference owned by the source compiler; resolution records the dependency whether the `ReferencePath` was persisted directly or derived by the compiler;
