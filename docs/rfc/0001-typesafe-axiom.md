...
- **Declarations and definitions are disjoint namespaces**: a symbol is a parameter *or* a derived value, never both. Collision — including through the record view (declaring `customer` as a record with a `turnover` field declares `customer.turnover`) — is a **constructor error**, before any call. Together with stripping, this makes shadowing *unrepresentable* rather than licensed: no binding can ever reach a definition, top-level or by descent. Upstream's shadowing feature is modeled in-language instead — an `Option`-typed parameter the definition consults (`riskFactorOverride: Number?`), explicit in the program, certified on both paths. The declared∧defined agreement check dies with the feature: there are no declared∧defined symbols left to agree.
