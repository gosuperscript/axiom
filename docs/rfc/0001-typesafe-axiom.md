...
| `OptionShape(inner)` | possibly-absent value | **Algebraic semantics**: denotes `None | Some(inner)`. Constructors remain observable when options nest. See laws below. |
...
*Option (algebraic semantics — these are theorems, not choices)*
- `Option<T>` denotes `None | Some(T)`. Therefore:
  - `T <: Option<T>` — a present value fills an optional slot.
  - `Option<Option<T>>` and `Option<T>` are distinct: the former additionally represents `Some(None)`. `Option<T> <: Option<Option<T>>`, but not the reverse.
...
- **Option layers in the resolution channel**: a compiled node's `Result<Option<mixed>>` carries its outer constructor; further constructors are nested `Option` values inside `Some`. This makes `None`, `Some(None)`, and `Some(Some(T))` representable. Ordinary source-compiler and operator interfaces still consume absence as one `null`: Axiom collapses nested layers before invoking them. Only structural option operations inspect the layers directly, and direct `Program` results expose the certified nesting.
...
- Record-shaped operand, declared optional property → `Option<PropertyType>`; an omitted property evaluates as `None`, while a present property occupies `Some` even when its value is itself absent.
- `Option<record-shaped>.property` → option-lifted property access. Each independent absence axis is retained, so accessing an optional option property produces `Option<Option<T>>` rather than collapsing omission and value absence.
