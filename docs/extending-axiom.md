...
This version supports any types for which the dialect owns `*`, not just core numbers. `applyIncludingAbsent()` evaluates both operands and invokes the bound operation. It therefore inherits the ordinary binary absence policy: an optional operand compiles only if the dialect contains a rule that resolves those optional operand types and handles `null`. The equality aliases have one syntax-directed exception during infix-expression typing: comparing an optional value with the absence-only `null` type elaborates directly to observation of the option constructor, so `Option<Unknown>` remains answerable. A known total operand is disjoint from absence and elaborates to the corresponding constant when no dialect rule explicitly owns that total/null pair; this fallback is how a compatibility dialect can preserve a published historical reading. Bare `Unknown` reaches ordinary resolution because no outer constructor is statically visible.
...
You cannot hand one back as a claim, because you are never given one to hand back. Nothing in the public surface produces an `ErrorType`: `CompiledSource::$returns` throws on a failed child, and `Diagnosis::$returns` is `null` for an expression whose root failed. Should one reach you by a route this library does not support, a type you claim through `produces()`, `custom()`, `constant()`, or declare on an operator rule is refused where you claim it. Absorb over a failed child instead; that is what it is for.
