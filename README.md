...
- **Equality**: `=`/`==`, `===`, `!=`, `!==` over domains supported by built-in value equality — numeric within `Number` (`1 == 1.0`), strict otherwise, `false` across bases (`===`/`!==` are aliases). During infix-expression typing, comparison with the absence-only `null` type elaborates structurally rather than becoming a value-equality overload. An optional counterpart observes only its option constructor, so even `Option<Unknown>` remains answerable. A known total counterpart is disjoint and therefore constant-false (constant-true when negated), unless a compatibility dialect explicitly owns that total/null pair. Bare `Unknown` stays inert, and equality between present opaque values still belongs to their owning package. Support is established before overlap is used for a *dead* compile diagnostic.
...
- **Authored default**: `??` — the present value, else the fallback. `T? ?? T` is `T` and `T? ?? T?` is `T?`, so `(roof_percentage ?? 0) > 0.25` types `Boolean` where the bare comparison types `Boolean?`. A fallback on a value that can never be absent is a *dead* refusal, so the operator only appears where absence is real. Unlike PHP's `??` it does not short-circuit — an infix node evaluates both children before applying the operation, so keep the fallback cheap and total
