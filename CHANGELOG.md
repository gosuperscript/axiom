...
- The compiler (`TypeInference`): literal-first inference, union element unification with exact list bounds, mandatory match exhaustiveness (`Never` vacuously covered; expression patterns compile like any program but never count toward coverage), shape-driven member access with reification (`TypeReifier`), optionality propagation, and `check` = infer + assignability. `TypeEnvironment` compiles symbols from declarations (boundary reads) and definitions (memoized slots); `LiteralTypeRegistry` types object literals. Every node compiles through the dialect's exact-class source compiler map: the core language's rules (`CoreSourceCompilers`) are registered by `Dialect::core()`, host `Source` classes through `Extension::sourceCompilers()` — the same seam, so the core rules hold no capability a host compiler lacks and an extension claiming a core source class meets the ordinary duplicate-ownership refusal. A source compiler receives its source and a `SourceCompilation` capability and returns the type claim and evaluation together; `SourceCompilation` recursively compiles owned child sources, resolves symbols in the current environment, binds typed infix and prefix operations from the composed dialect, and types embedded PHP values literal-first — exactly the language's own semantics, with no runtime dispatch restored. Host `Source` objects remain serializable data descriptions; live collaborators stay in the extension and are captured only in the compiled program.
