...
### Added
- RFC 0001: Typesafe Axiom (`docs/rfc/0001-typesafe-axiom.md`) — the accepted design for the statically typed, compiled expression language, shipping as one breaking release; `docs/extending-axiom.md` is the extension guide.
...
- Typed operators, one verdict: `OperatorOverloader`/`UnaryOverloader` are single-method contracts — `resolve(operator, operand types)` returns a `ResolvedOperation` carrying the return type *and* the evaluation, so a rule's static and runtime semantics are one statement and cannot drift. Core arithmetic, ordering, and logic are dialect rows; equality (`EqualityOverloader`, with overlap-based dead findings and value equality), membership (`has`/`in`), and intersection are hand-written type functions sharing `SetOperands` and `ShapeDomain::all`.
...
- `Dialect` + `Extension`: the operator rules and literal registry live in one value object consumed at compile time; the program embeds what the dialect resolved, so checking with different rules than you run is unrepresentable. **Ambiguity is refused, never ranked**: two rows for one operator with overlapping operand types are a `Dialect` construction error; multiple rules resolving the same node are a compile error naming both. List order decides nothing. Duplicate literal registrations stay loud errors.
- The compiler (`TypeInference`): literal-first inference, union element unification with exact list bounds, mandatory match exhaustiveness (`Never` vacuously covered; expression patterns compile like any program but never count toward coverage), shape-driven member access with reification (`TypeReifier`), optionality propagation, and `check` = infer + assignability. `TypeEnvironment` compiles symbols from declarations (boundary reads) and definitions (memoized slots); `LiteralTypeRegistry` types object literals; `TypedSource::compile()` is the host-source seam — the type claim and the evaluation in one statement, so a host cannot register behavior its claim does not describe.
...
- The totality harness and the admission-honesty law: for every rule of the composed dialect, generative specimens of every certified operand-type pair must evaluate without escaping, into the resolved return type; for every `Type`, `coerce` output must pass `assert` (census-enforced) — together the runtime trust chain of compile-then-trust. The shape-soundness census keeps its specimen (C1) and shape-truth (C2) laws.
...
- **The resolver layer is deleted.** `Resolvers/*` (including `DelegatingResolver` and its class map), `Patterns/*` (`PatternMatcher` and the matcher registry), and the per-call `Context` are gone — nodes compile to their evaluations, and the `illuminate/container`/`psr/container` dependencies fall away. `Expression` loses its `$resolver` constructor parameter. First-party node semantics are no longer host-swappable: the compiler certifies semantics, so silently replacing them was a drift channel, not a feature. Custom sources implement `TypedSource::compile()` (replacing `TypedSource::returnType()` + a separately registered `Resolver` — two faces nothing verified against each other).
- **The operator contract collapses.** `supportsOverloading`/`evaluate`/`handles`/`typeOf` (four methods, two faces) become `resolve()` (one verdict). `BinaryOverloader`, `ComparisonOverloader`, `LogicalOverloader`, `NotOverloader`, `NegateOverloader`, and `DefaultOverloader` no longer exist as classes — their semantics live in core dialect rows and `EqualityOverloader`. `NullOverloader` (the `null ∘ null → null` rule) is deleted outright: it deliberately certified nothing, which under compile-then-trust means it could never run; absence policy is spelled as a rule that resolves `Option`-shaped operand types and refuses present pairs. **Migration**: signature-built extension rules are unchanged; hand-written rules re-state themselves as one `resolve()` method (see the extension guide).
